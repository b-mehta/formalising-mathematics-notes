...
If ``G`` and ``H`` are groups, then to give a group homomorphism from ``G`` to ``H`` is to give a function from ``G`` to ``H`` which preserves multiplication, so a term of type ``GroupHom G H`` is a *pair*, consisting of a function and a proof. But what does that ``[Group G]`` mean and why is it in weird brackets?
